...
const Projects = () => {
  ...
  // no we r creating navigate function using which we can navigate users to next page
  const navigate = useNavigate()
  // now we need to create the states where we will save the project data
  const [project, setProject] = useState<Project | null>(null)
  const [loading, setLoading] = useState(true)
  // setLoading here is the setter function
  ...
  const previeRef = useRef<ProjectPreviewRef>(null)
  ...
  const downloadCode = () => {
    const code = previeRef.current?.getCode() || project?.current_code;
    if(!code){
      if(isGenerating){
        return
      }
  ...
  return project ? (
    <div className="flex flex-col h-screen w-full bg-gray-900 text-white">
      {/* builder navbar  
      this piece of code is related with last few lines of code in index.css which help to hide the scrollbar*/}
      <div className="flex max-sm:flex-col sm:items-center gap-4 px-4 py-2 no-scrollbar">
        ...
        <div className="flex items-center justify-end gap-3 flex-1 text-xs sm:text-sm">
          ...
          <button
            ...
            className="bg-linear-to-br from-blue-700 to-blue-600 hover:from-blue-600 hover:to-500 text-white px-3.5 py-1 flex items-center gap-2 rounded sm:rounded-sm transition-colors"
          >
      ...
      <div className="flex-1 flex overflow-auto">
        ...
        <div className="flex-1 p-2 pl-0">
          <ProjectPreview ref={previeRef} project={project} isGenerating={isGenerating} device={device}/>
        </div>
      </div>
    </div>
  ) : (
