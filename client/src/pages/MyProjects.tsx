...
const MyProjects = () => {
    ...
    // here we have added the project state where we can add the project data 
    ...
    const fetchProjects = async () =>{
      ...
      // Simulate loading 
       setTimeout(()=>{
        setLoading(false)
                      // after one sec the loading will be removed 
       },1000)

    }  
    
    // here we created the deleteproject and connected with delete icon
    const deleteProject=async(projectId:string) =>{

    }

    useEffect(()=> fetchProjects()        
      //   whenever this function is loaded it will call the baove function
      ,[])
  return (
    <>
      <div className='px-4 md:px-16 lg:px-24 xl:px-32'>
          {loading ? (
              ...
              // it will be displayed when the loading is true if it is false it will check for the projects length
        ):projects.length>0 ?(
          <div className='py-10 min-h-[80vh]'>
              <div className='flex items-center justify-between mb-12'>
                ...
                {/*it will align the text in the left side and the text in the right side
                    ?- above signifies the ternary operator 
                */}
              </div> 
                <div className='flex flex-wrap gap-3.5'>
                  {projects.map((project)=>(
                    //  to create unique id we will add a key inside div
                    <div onClick={()=> navigate(`/projects/${project.id}`)}key={project.id} className='relative group w-72 max-sm:mx-auto cursor-pointer bg-gray-900/60 border border-gray-700 rounded-lg overflow-hidden shadow-md group hover:shadow-indigo-700/30 hover:border-gray-800/80 transition-all duration-300'>
                      ...
                      :(
                        ...
                        {/* if the current code is available we can generate the preview using this current code */}

                      </div>
                      {/* Content  below code is used to create the webstite button along with the title of projects */}
